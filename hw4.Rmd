...
required_packages <- c("microbenchmark", "tidyverse")
...
solve_via_cholesky <- function(A, b){
  L <- chol(A)
  sol <- backsolve(L, forwardsolve(t(L), b))
return(sol)
}


solve_via_QR <- function(A, b){
  sol = qr.solve(A, b, tol = .Machine$double.eps)
  return(sol)
}
...
Cholesky decomposition had the fastest median time, followed closely by UL decomposition. QR decomposition had the slowest time by far.

These speeds somewhat agree with what I expect from the complexity analysis. UL decomposition was supposed to take roughly double the time as Cholesky ($2/3*p^3$) vs ($1/3*p^3$), but the speeds were much closer than expected. This was likely due to the small condition number of A. 
QR decomposition takes the longest, which is expected due to the computing cost of ($2p^3$).
...
matvec_double_out = matvec_double(A, x)
matvec_ldouble_out = matvec_ldouble(A, x)

#Comparing l2 norm

l2_norm_diff = calc_rel_error(matvec_ldouble_out, matvec_double_out, "norm")
print(l2_norm_diff)

#Comparing coordinate-wise relative diffs
coord_wise_diff_5 = calc_rel_error(matvec_ldouble_out, matvec_double_out, "five_percentile")
coord_wise_diff_med = calc_rel_error(matvec_ldouble_out, matvec_double_out, "median")
coord_wise_diff_95 = calc_rel_error(matvec_ldouble_out, matvec_double_out, "ninety_five_percentile")

c(coord_wise_diff_5, coord_wise_diff_med, coord_wise_diff_95)
```

## Answer:

Yes, the magnitudes of the errors are quite small, which is what I would expect with a condition number of only 1e3.
...
l2norm_LU = calc_rel_error(x, sol_LU, metric = "norm")
l2norm_chol = calc_rel_error(x, sol_chol, metric = "norm")
l2norm_qr = calc_rel_error(x, sol_qr, metric = "norm")

df_plot = data.frame("Algorithm" = c("LU", "Cholesky", "QR"), "l2_norm" = c(l2norm_LU, l2norm_chol, l2norm_qr))

ggplot(data = df_plot, aes(x = Algorithm, y = l2_norm)) + geom_point(color = "blue", size  = 3) + geom_line()
```

## Answer:
Cholesky decomposition is more accurate than the others. LU is close in second place, but QR decomposition is much more inaccurate than the other two.
...
n_sim <- 32L
mat_size <- 512L
cond_num <- c(1e6, 1e9, 1e12)
metrics <- c("norm", "median", "five_percentile", "ninety_five_percentile", "l1norm")

rel_error_list_1e6 <- rel_error_list_1e9 <- rel_error_list_1e12 <- lapply(
  c("lu", "chol", "qr"), 
  function(method) pre_allocate_error_list(n_sim, metrics)
)

for (sim_index in 1:n_sim) {
  A1 <- rand_positive_def_matrix(mat_size, cond_num[1])
  A2 <- rand_positive_def_matrix(mat_size, cond_num[2])
  A3 <- rand_positive_def_matrix(mat_size, cond_num[3])
  x <- rnorm(mat_size) 
  b1 <- matvec_ldouble(A1, x)
  b2 <- matvec_ldouble(A2, x)
  b3 <- matvec_ldouble(A3, x)
  
  x_approx1 <- list( 
    lu = solve(A1, b1),
    chol = solve_via_cholesky(A1, b1),
    qr = solve_via_QR(A1, b1)
  )
  x_approx2 <- list( 
    lu = solve(A2, b2),
    chol = solve_via_cholesky(A2, b2),
    qr = solve_via_QR(A2, b2)
  )
  x_approx3 <- list( 
    lu = solve(A3, b3),
    chol = solve_via_cholesky(A3, b3),
    qr = solve_via_QR(A3, b3)
  )
  ...
  for (method in c("lu", "chol", "qr")) {
    rel_error1 <- lapply(
      metrics, 
      function (metric) calc_rel_error(x, x_approx1[[method]], metric)
    )
    names(rel_error1) <- metrics
    for (metric in names(rel_error1)) {
      rel_error_list_1e6[[method]][[metric]][sim_index] <- rel_error1[[metric]]
    }
  }
  ...
  for (method in c("lu", "chol", "qr")) {
    rel_error2 <- lapply(
      metrics, 
      function (metric) calc_rel_error(x, x_approx2[[method]], metric)
    )
    names(rel_error2) <- metrics
    for (metric in names(rel_error2)) {
      rel_error_list_1e9[[method]][[metric]][sim_index] <- rel_error2[[metric]]
    }
    }
    ...
    rel_error3 <- lapply(
      metrics, 
      function (metric) calc_rel_error(x, x_approx3[[method]], metric)
    )
    names(rel_error3) <- metrics
    for (metric in names(rel_error3)) {
      rel_error_list_1e12[[method]][[metric]][sim_index] <- rel_error3[[metric]]
    }
  }
...
df_lu_1e6 = data.frame(condition_num = "1e6", method = "lu", rel_error_list_1e6[["lu"]])
df_chol_1e6 = data.frame(condition_num = "1e6", method = "chol", rel_error_list_1e6[["chol"]])
df_qr_1e6 = data.frame(condition_num = "1e6", method = "qr", rel_error_list_1e6[["qr"]])


df_lu_1e9 = data.frame(condition_num = "1e9", method = "lu", rel_error_list_1e9[["lu"]])
df_chol_1e9 = data.frame(condition_num = "1e9", method = "chol", rel_error_list_1e9[["chol"]])
df_qr_1e9 = data.frame(condition_num = "1e9", method = "qr", rel_error_list_1e9[["qr"]])


df_lu_1e12 = data.frame(condition_num = "1e12", method = "lu", rel_error_list_1e12[["lu"]])
df_chol_1e12 = data.frame(condition_num = "1e12", method = "chol", rel_error_list_1e12[["chol"]])
df_qr_1e12 = data.frame(condition_num = "1e12", method = "qr", rel_error_list_1e12[["qr"]])


df_combined = rbind(df_lu_1e6, df_chol_1e6, df_qr_1e6, df_lu_1e9, df_chol_1e9, df_qr_1e9, df_lu_1e12, df_chol_1e12, df_qr_1e12) %>%
  pivot_longer(cols = all_of(metrics), names_to = "metric", values_to = "err")



df_combined %>%
  group_by(condition_num, metric, method) %>%
  summarize(mean(err), median(err), max(err), min(err))


ggplot(data = df_combined[df_combined$condition_num == "1e6",], aes(x = metric, y = err, group = method, color = method)) + geom_point() + labs(title = "Errors with condition number 1e6")

ggplot(data = df_combined[df_combined$condition_num %in% c("1e6", "1e9"),], aes(x = metric, y = err, group = method, color = method)) + geom_point() + facet_wrap(vars(condition_num)) + labs(title = "Errors with condition numbers 1e6 and 1e9")

ggplot(data = df_combined, aes(x = metric, y = err, group = method, color = method)) + geom_point() + facet_wrap(vars(condition_num)) + labs(title = "Errors with condition number 1e6, 1e9, and 1e12")
```

## Answer:

Consistently, the largest errors arise when using QR decomposition with any metric. Overall, the metric that provides the largest errors is the 95th percentile relative difference. Finally, larger condition numbers for the A matrix create exponentially larger errors, with the largest error (using QR decomp, 95th percentile rel. diff, and condition number 1e12) approaching 0.00125.
...
* [Link to the pull request.](https://github.com/sfansler/hiperglm/pull/4)
