...
**Note:** This function directly modifies `y` as the variable is passed _by reference_ via "`&`."
<p style="margin-top: -1ex;"> </p>
```{r}
y <- rep(0.00159, n)
...
<!-- Discuss "not general" matrices (symmetric, triangular, banded, etc) and what "X" and "d" is for. -->
...
<!-- Mention CPU also include floating point unit separately from ALU. -->
...
Easy enough to see why dividing an unsigned integer by two amounts to a logical shift.

--

Division by two for a signed integer is a bit more involved.
...
The "sar" operation makes sense for keeping the sign.
But what's up with the preceding 31-bit logical shift?

--

The "shr" followed by "add" amounts to "+1 if positive."
...
.pull-left[
  ...
  This ensures consistent rounding towards 0; e.g. <br>
  "&plusmn; 3 / 2" returns "&plusmn; 1."
]
.pull-right[
...
# Why should we care about assembly code?

It's used in real life;&thinsp; e.g. openBLAS implementation of `gemv` in the [x86_64](https://github.com/OpenMathLib/OpenBLAS/blob/develop/kernel/x86_64/dgemv_n.S) and [ARM](https://github.com/OpenMathLib/OpenBLAS/blob/master/kernel/arm64/gemv_n.S) instructions sets.

---

# Why should we care about assembly code?

Knowing a bit could help you identify big opportunities:

![:vspace -1ex]
![](figure/computing_at_low_level/eigen_faq_header.png)
![:vspace -2.5ex]
$$\vdots$$
![:vspace -1.5ex]
![](figure/computing_at_low_level/eigen_faq_checking_vectorization.png)

<!-- https://eigen.tuxfamily.org/index.php?title=FAQ#How_can_I_check_that_vectorization_is_actually_being_used.3F -->
...
# Consequences of data motion bottleneck

Here are some consequences of data motion being computational bottleneck.

--

If multiplying a bunch of vectors by the same matrix, first stacking them into a matrix makes the calculation faster:

```{python, eval=FALSE}
import numpy as np
n, p, k = 4096, 1024, 16
X = np.random.randn(n, p)
b_list = [np.random.randn(p, 1) for _ in range(k)]

# Multiply one vector at a time
for l in range(len(b_list)): X @ b_list[l]

# Multiply them all at once
B = np.hstack(b_list)
X @ B 
```

<!-- B_col_major = np.array(np.random.randn(p, k), order = 'F') -->
<!-- for l in range(len(b_list)): X @ B_col_major[:, l] -->

---

# Consequences of data motion bottleneck

You can also observe the same effect in R if you have an installation with good linear algebra library:

```{r, eval=FALSE}
n <- 4096; p <- 1024; k <- 16
X <- matrix(rnorm(n * p), nrow = n, ncol = p)
b_list <- lapply(1:k, function(dummy) rnorm(p))
...
B <- do.call(cbind, b_list) # Or use \`dplyr::bind_cols()` 
bench::mark(X %*% B)
```
...
bench::mark(
  for (l in 1:k) {
    dgemv_eigen(X, b_list[[l]])
  }
)
bench::mark(dgemm_eigen(X, B))
```
...
Multiplying matrix with its own transpose can be faster than multiplying two different matrices:
