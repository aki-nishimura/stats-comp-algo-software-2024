...
```{r, include=FALSE}
# Hack to format the error message
knitr::knit_hooks$set(error = function(x, options) {
  "<pre style=\"color: #DA0000;\"><code>Error in lm.fit(x, y, offset = offset, singular.ok = singular.ok, ...): <br>singular fit encountered
  </code></pre>"
})
```
...
Except for one coef estimate being `NA`:
![:vspace -1ex]
...
Think of a _problem_, machine implementation of which is an _algorithm_, as a map: $\, \bx \to f(\bx)$ (e.g. $\boldsymbol{x} \to \boldsymbol{A}^{-1} \boldsymbol{x}$).

--

**Question:** How sensitive is a problem $f(\bx)$ to small perturbation in the input $\bx$?
...
$$
* $\bm{V} \in \mathbb{U}^{n \times p}$ and $\bm{U} \in \mathbb{R}^{p \times p}$ orthogonal;
* $\boldsymbol{\Gamma} = \textrm{diag}(\gamma_1, \ldots, \gamma_p) \,$ for $\, \gamma_1 \geq \ldots \geq \gamma_p \geq 0$.

--

**Note:** ![:vspace -.75ex]
* $\gamma_1 = \displaystyle \max_{\| \bv \| = 1} \| \bA \bv \| = \| \bA \|$ where $\| \bA \|$ is the _matrix norm_.
--
...
![:vspace -2ex]
## Note:
* $\| \bA^{-1} \| = \gamma_{\min}^{-1}(\bA)$
* $\kappa(\bA) = \gamma_{\max}(\bA) / \gamma_{\min}(\bA)$, the ratio of the largest to smallest eigen/singular values. 
...
.pull-left[
  ![:vspace 2.2ex]
  ![:image 100%](figure/reference/sci_computing_intro_survey_by_michael_heath.jpeg)
]

.pull-right[
  ![:image 90%](figure/reference/trefethen_and_bau_linalg_book.jpg)
]