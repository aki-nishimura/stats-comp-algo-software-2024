...
[user]
        ...
        email = 21285656+acolum@users.noreply.github.com
[filter "lfs"]
...
**Your answer:** The size of the folder doesn't look reasonable relative to the sizes of the individual files inside it because Git stores the entire history of the project, including every version of every file. This means that if we have a large file and we make a small change to it, Git has to store the entire file again in its history, which can quickly add up if we’re making many changes to large files like we did in this exercise. Also, even if the current version of the files is small, the repository (overall) can still be large if it has a long history.

The difference in the disk space usages under the two cases above can be explained by the fact that in the first case, we were committing both the TeX source file and its PDF output, while in the second case, we were only committing the TeX source file. PDF files are usually larger than TeX files, so committing them in addition to the TeX files greatly increases the size of the repository. This is why it’s generally a good idea to avoid adding non-source files, especially large binary files (like PDFs), to a Git repository. 
...
**Your answer:** We observe a triangular oscillating pattern (akin to a sawtooth wave), where the repository size (in MB) increases rapidly then suddenly decreases multiple times as the commit count progresses. 

The increases in the graph represent the growth of the repository size as new commits are made. This is expected because each commit adds a new version of the file to the repository's history, which increases its size. The sudden decreases in the graph could be due to Git's garbage collection running automatically, which can remove objects that are no longer necessary and compress objects to reduce disk space. The pattern of increasing and decreasing repository sizes illustrates a balance between repository growth  (due to new commits) and optimization (due to garbage collection).

As for the question about why GitHub discourages committing large binary files, the reason remains the same. Binary files are not diffable, meaning that Git must store the entire file every time it changes. This can quickly inflate the size of the repository. Moreover, large binary files can make cloning and fetching from the repository slow and use up a lot of disk space quickly. Even though Git has mechanisms like garbage collection to optimize the repository, it’s still a best practice to avoid committing large binary files to keep the repository manageable and efficient.
...
plot(
  ...
  y = repo_size/1000, # Convert KB printed by du to MB
...
1. Completed `edit_and_commit.sh`: Please see `git_experiment/edit_and_commit.sh` for my edited script.
2. Answer to the question above, along with the plot: Please see above for my answer to the question, and compile this Rmd document to see the plot.
...
- Hyperlink to the `hiperglm` repository on your Github account: <https://github.com/acolum/hiperglm>
- Screenshot of the package's Git history visualized through a Git GUI of your choice: I've added a screenshot named `git_history_screenshot.png` to the `figure` folder.
