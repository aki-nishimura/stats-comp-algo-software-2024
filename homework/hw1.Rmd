...
Folder size (with pdfs): 75K
  Individual file sizes: 227 edit_and_commit.sh
                         22 foo.aux
                         3.4K foo.log
                         36K foo.pdf
                         337 foo.tex
                         
Folder size (without pdfs): 19K
  Individual file sizes: 227 edit_and_commit.sh
                         337 foo.tex

Disk space with pdfs: 1.1M
Disk space without pdfs: 495K


The folder size with the pdf included is 75K, while the individual file sizes inside that folder sum to around 40K. I believe this is because the git repo additionally stores version history metadata which takes up extra space. Similarly, the folder size without the pdf is 19K while the individual file sizes sum to 564. This is likely due to similar reasons. 

For the disk space difference, this is because the folder including the pdf takes up much more space. The pdf is a binary file, which takes up more space than the TeX file, which is a text file. This is why the folder size as well as the disk space usage is larger with the folder including the pdf.
...
`rsync -P "your_username@jhpce03.jhsph.edu:/users/your_username/stats-comp-algo-software-2023/homework/git_experiment_copy/git_repo_size_history.txt" git_experiment` (assuming that you are running the command from the `homework` folder).
...
The plot below shows a large fluctuation of disk usage as the number of commits increases. This is due to how Git stores version histories and metadata. For example, Git sometimes creates pack files to compress objects and create more space as the number of commits increases. This is done when there are a large number of loose files, Git recognizes this and packs them into a single binary file. [Source](https://git-scm.com/book/en/v2/Git-Internals-Packfiles#:~:text=However%2C%20occasionally%20Git%20packs%20up,push%20to%20a%20remote%20server.)

Based off these results, it is clear why Github discourages use of large binary files. Text files are much smaller, which is easier for Git to keep track of changes and perform effective version control. Large binary files also increase the size of the repository, which could slow down cloning and pulling/pushing.

```{r}
getwd()
data = read.table("git_repo_size_history.txt")

sizes = data[,1]
commit_num = seq(1:length(sizes))
...
[Link to hiperglm repository](https://github.com/sfansler/hiperglm)  
  
- Screenshot of the package's Git history visualized through a Git GUI of your choice

<img src="git_history_screenshot.png" alt="Place the screenshot here"/>