...
```{bash, eval=FALSE}
# Paste the output of `cat .git/config` here
...
[submodule]
	active = .
[remote "origin"]
	url = https://github.com/m-qin/stats-comp-algo-software-2024.git
...
[lfs]
	repositoryformatversion = 0
[remote "upstream"]
	url = https://github.com/aki-nishimura/stats-comp-algo-software-2024.git
	fetch = +refs/heads/*:refs/remotes/upstream/*
[branch "hw1"]
	remote = origin
	merge = refs/heads/hw1
```
...
```{bash, eval=FALSE}
git clone https://github.com/m-qin/stats-comp-algo-software-2024
```
...
**Notes**: `-i` means modify in place, and "i" means insert line.
...
Alternatively, you can edit using the text editor `nano edit_and_commit.sh`.
...
**Results (adding foo.pdf to git):**

```{bash, eval=F}
[compute-103 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ du -h -s .
988K	. # Hmm I think this number is increasing each time I delete the repository and recreate it; it was 712K earlier
[compute-103 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ ls -l -h
total 75K
-rw-r----- 1 mqin users  394 Feb  1 12:43 edit_and_commit.sh
-rw-r----- 1 mqin users   32 Feb  1 12:38 foo.aux
-rw-r----- 1 mqin users 3.4K Feb  1 12:38 foo.log
-rw-r----- 1 mqin users  36K Feb  1 12:38 foo.pdf
-rw-r----- 1 mqin users  338 Feb  1 12:37 foo.tex
[compute-103 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ ls -l -h -a
total 132K
drwxr-x--- 3 mqin users    8 Feb  1 12:40 .
drwxr-x--- 7 mqin users   15 Jan 29 21:37 ..
-rw-r----- 1 mqin users  394 Feb  1 12:43 edit_and_commit.sh
-rw-r----- 1 mqin users   32 Feb  1 12:38 foo.aux
-rw-r----- 1 mqin users 3.4K Feb  1 12:38 foo.log
-rw-r----- 1 mqin users  36K Feb  1 12:38 foo.pdf
-rw-r----- 1 mqin users  338 Feb  1 12:37 foo.tex
drwxr-x--- 8 mqin users   13 Feb  1 12:43 .git
```

Next, make a fresh clone of the repo; you can remove the old one via `rm -rf stats-comp-algo-software-2024`. (`-rf` is short for `-r -f`.)
And repeat the above process from the beginning, but without ever adding the PDF output to the repo.

**Results (not adding foo.pdf to git):**

```{bash, eval=F}
[compute-153 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ du -h -s .
623K	.
[compute-153 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ ls -l -h
total 75K
-rw-r----- 1 mqin users  386 Feb  1 22:41 edit_and_commit.sh
-rw-r----- 1 mqin users   32 Feb  1 22:41 foo.aux
-rw-r----- 1 mqin users 3.4K Feb  1 22:41 foo.log
-rw-r----- 1 mqin users  36K Feb  1 22:41 foo.pdf
-rw-r----- 1 mqin users  337 Feb  1 22:41 foo.tex
[compute-153 /users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy]$ ls -l -h -a
total 132K
drwxr-x--- 3 mqin users    8 Feb  1 22:41 .
drwxr-x--- 7 mqin users   16 Feb  1 22:39 ..
-rw-r----- 1 mqin users  386 Feb  1 22:41 edit_and_commit.sh
-rw-r----- 1 mqin users   32 Feb  1 22:41 foo.aux
-rw-r----- 1 mqin users 3.4K Feb  1 22:41 foo.log
-rw-r----- 1 mqin users  36K Feb  1 22:41 foo.pdf
-rw-r----- 1 mqin users  337 Feb  1 22:41 foo.tex
drwxr-x--- 8 mqin users   13 Feb  1 22:41 .git
```
...
**Your answer:** The folder size as reported by `du -h -s` is multiple times larger than the total size of all files in it as reported by `ls -l -h -a` (988 KB vs. 132 KB); I am not sure why. Meanwhile, the folder size without committing the pdf file is about 2/3 times the size of the folder size with committing the pdf file (988 KB vs. 623 KB); this makes sense to me because git can store less information if I don't commit the pdf file. However, the `.git` folder seems to be 13 (bytes?) in both cases, so I am a bit confused.
...
`rsync -P "mqin@jhpce03.jhsph.edu:/users/mqin/stats-comp-algo-software-2024/homework/git_experiment_copy/git_repo_size_history.txt" git_experiment` (assuming that you are running the command from the `homework` folder).
...
**Your answer:** Interestingly, my git repository size goes up then down periodically. A quick ChatGPT query suggested that git may be performing routine garbage collection as my repo size increases. I'm also puzzled as to why my .txt files seems to only have 6,601 lines. Did the garbage collection delete stuff?
...
plot(
  ...
  y = repo_size / 1000, # du seems to print in KB, so convert to MB
...
```{r pkg-sample-code, eval=F, echo=F}
# see also my handwritten notes

# from biostat 777 project 2
make_ci_class <- function(x){
  structure(list(x = x), class = "ci_class")
}
...
#' @export
predict.hlogit <- function(hlogit, ...){ # at least, I think I need ...
