...
We observe that the disk usage of case 2 is almost half of case 1. This is because case 1 involves committing and saving each copy of the pdf files, which is around 30K+ on itself. Each snapshot of case 1 folder takes more space than case 2.
...
*Observation*: The plot below shows that the repo size fluctuates. In each cycle of about 2000 commits, the repo size increases linearly, then suddenly drops to a relatively low level. After this sudden drop, we enter the next cycle of increase then sudden drop in repo size. So it looks like there is a limit to how big the repo will grow and git manages to cut down the repo size in every 2000+ commits. Nonetheless, committing TeX file increases the repo size fast and can use a lot of storage. The repo size even after the sudden drop is still high.

*Explanation*: This fluctuation in repo size is be due to Git's packfile creation mechanism. In each cycle, one line is added to TeX, changes committed, and Git stores the changes. The new files thus accumulates. When the repo size gets too large, Git thinks it is more efficient to compress all the changes thus far into one single packfile, which is especially efficient considering that consecutive files only differs by one line. The repo size drops after packfile creation. To verify that this is the case, I have found these packfiles in my repo. There are 4 packfiles, corresponding to the 4 sudden drops in repo size as shown in the figure below. Nonetheless, the overall trend over the 10,000 commits is that the repo size is going up fast despite packfile creation.
