...
**Your answer:** The size of the folder does not look reasonable relative to the sizes of the individual files inside it.  To further investigate the matter, I utilized the function "du -h -a git_experiment", and that showed that the git-substructure was responsible for the inflated disk-space. Git also stores metadata about the repository in the Git folder, leading to further inflation. 

It is further noted that the space usage is worse for the case where the pdf is commited compared to the case where it is not. It is important to note that for binary files( such as pdfs) Git stores every change to the repository as a separate object. Since we are doing 10 changes, there are generally 10 different versions of the pdf stored which in turn lead to an even larger inflation.This problem does not occur at such a scale in tex files, because git tracks just the changes done in the tex files, and does not explicitly need to keep copies of the tex files after each iteration.
...
**Your answer:** The following plot relating the disk usage of the git repo with the number of commits on the $x$-axis shows that the disk usage of the git-repo demonstrates an oscillatory pattern with the number of commits. There are increases and decreases which are shown in the pattern below

<img src="https://github.com/achatto4/stats-comp-algo-software-2024/blob/main/homework/figure/Capture1.1.JPG?raw=true" alt="Place the screenshot here"/>

```{r, eval=FALSE}
repo_size <- scan(file.choose())/1000 #Convert to MB
plot(
  x=1:length(repo_size), y=repo_size,
  frame = FALSE, # Remove the ugly box
  xlab = "Commit counts",
  ylab = "Repo size (MB)"
)
```
Reason for increasing nature- The primary contributors to the escalating disk usage are the accumulation of versioned files and their associated historical data. As the number of commits grows, each commit introduces new changes and potentially additional files, resulting in a gradual expansion of the repository's footprint on disk. Moreover, large binary files, frequent modifications, or the retention of obsolete or redundant data can further contribute to the overall increase in disk usage over the course of the repository's development history. 

Reason for decreasing nature- Version control systems like Git typically use various compression techniques to optimize storage. When you commit changes, Git may initially increase the disk space as it saves a new snapshot of the file. However, over time, Git performs periodic optimizations, and this could lead to a decrease in disk space. Git uses delta compression and other mechanisms to minimize redundancy in storage. If there are repetitive patterns or similarities between versions, Git might identify and store them more efficiently. These happen alternatively hence giving us the observed oscillatory structure.

Furthermore, the two main reasons why Github discourages us from commiting (large) binary files are understood to be the follows-

Storage and Bandwidth: Large binary files significantly increase the storage requirements of a repository, due to lack of ease of reproducibility. Storing and transferring such files can consume more disk space and bandwidth, impacting both GitHub's infrastructure costs and the user experience for those cloning or fetching the repository.

Performance: Handling large binary files can degrade the performance of Git operations, particularly cloning and fetching, as the entire history, including binary files, must be downloaded and processed.
...
---- https://github.com/achatto4/hiperglm

- Screenshot of the package's Git history visualized through a Git GUI of your choice

<img src="https://github.com/achatto4/stats-comp-algo-software-2024/blob/main/homework/figure/Capture.JPG?raw=true" alt="Place the screenshot here"/>
