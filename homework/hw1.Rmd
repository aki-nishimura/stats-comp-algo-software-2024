...
Next, make a fresh clone of the repo; you can remove the old one via `rm -rf stats-comp-algo-software-2023`.
And repeat the above process from the beginning, but without ever adding the PDF output to the repo.

**Tips:** You can edit the bash script locally and transfer to JHPCE via `rsync -P git_experiment/edit_and_commit.sh your_username@jhpce03.jhsph.edu:/users/your_username/`.
...
***PDF file takes more space compared to just text files, so not creating a pdf in the latter case would utilize less space.***
...
**Tips:** You can transfer the disk usage result from JHPCE to your local computer via `rsync -P "your_username@jhpce03.jhsph.edu:/users/your_username/stats-comp-algo-software-2023/homework/git_experiment_copy/git_repo_size_history.txt" git_experiment` (assuming that you are running the command from the `homework` folder).
...
**Your answer:** ***With each line added, more space is taken up by the repo in a linear fashion as observed in the plot.***
...
repo_size <- scan("git_experiment/git_repo_size_history.txt",
                   what = "character")
repo_num <-  as.numeric(gsub("K", "", repo_size))

pdf("git_experiment/plot.pdf")
p<- plot(
  repo_num,
...
1.  Completed `edit_and_commit.sh : under git_experiment, copying code here.`
...
2.  Answer to the question above, along with the plot
    ...
    ***plot is under git_experiment as a pdf.***
...
[package link](https://github.com/sparthib/hiperglm): <https://github.com/sparthib/hiperglm>

-   Screenshot of the package's Git history visualized through a Git GUI of your choice![](images/Screen%20Shot%202024-02-02%20at%203.43.34%20PM.png)
