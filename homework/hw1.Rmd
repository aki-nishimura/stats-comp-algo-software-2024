...
*Observation*: The plot below shows that the repo size fluctuates where each cycle is about 2000+ commits. In each cycle, the repo size increases linearly, then suddenly drops to a size close to the repo size after init commit (0.25M+). After this sudden drop, we enter the next cycle of 2000+ commits showing linear increase in repo size at first, then sudden drop in repo size level. So it looks like there is a limit to how big the repo will grow since we observe that git manages to cut down the repo size in every 2000+ commits. Nonetheless, committing TeX file increases the repo size fast and can use a lot of storage. 
...
In part A we see that when including pdf in the commits, the total repo size after 10 commits is 1MB while the repo size is 505K without including pdf in the 10 commits. This is because the pdf file (the large binary file) itself takes up too much space which causes each commits to increase faster than without including pdf. If we commit the pdf 10,000 times, even with the packfile mechanism, the repo size will be much larger than not committing the pdf, and the pdf will not compress really well into packfile.

Therefore, GitHub discourages committing large binary files because large binary files take up too much storage space of the version control repo. Making changes to the binary file and committing again can increase the repo size very quickly, even with the packfile mechanism. And in the example of pdf, it is also harder to merge changes to pdf between different commits, making collaboration harder.
