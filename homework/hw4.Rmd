...
The discrepancies between our theoretical expectations and the actual results above are probably due to the unique optimization algorithms used for each method. These optimizations might prevent the full decompositions from being calculated, thereby reducing the computational time used. This shows that the actual performance of these algorithms can vary depending on the specific implementation and the nature of the data they are applied to. Hence, it’s always a good idea to benchmark the performance of different algorithms in the specific contexts in which they're used.
