...
solve_via_qr <- function(A,b){
  # qr_decomp <- qr_wrapper(A)
  # R <- qr_decomp$R
  # Q <- qr_decomp$Q
  # 
  # return(solve(R) %*% t(Q) %*% b)
  return(qr.solve(A,b))
}
bench::mark(
  solve_via_qr(A,b)
...
The fastest algorithm was using LU decomposition, and the slowest was Cholesky. Since we had a square matrix, Cholesky is expected to be O(p^3) and QR is expected to be O(2np^2) which is O(p^3). This is surprising because QR actually was faster than Cholesky in our benchmark.
...
**Questions:** What is the relative difference in $\ell^2$-median?
...
# Fill in
db <- matvec_double(A,x)
ldb <- matvec_ldouble(A,x)

abs((norm(db, type="2") -norm(ldb, type="2"))/norm(ldb, type="2"))

head(abs((db - ldb)/ldb))

```

The relative difference in the L2 median is 0, suggesting these two methods yield the same or almost the same results. However, the coordinate-wise relative differences are non-zero albeit very small, suggesting that there are still differences between the two. The observed magnitudes are about what we would expect because they are on the level of precision of the long doubles.
...
qr_sol <- solve_via_qr(A,b)
chol_sol <- solve_via_cholesky(A,b)
LU_sol <- solve(A,b)

qr_norm <- norm(qr_sol - x, type="2")
chol_norm <- norm(chol_sol - x, type="2")
LU_norm <- norm(LU_sol - x, type="2")

df <- as.data.frame(cbind(type=c("QR", "CHOL", "LU"), error=as.numeric(c(qr_norm, chol_norm, LU_norm))))
library(ggplot2)
ggplot(df, aes(x=type, y=as.numeric(error)))+
  geom_bar(stat="identity")+
  ylab("L2 norm between solved vector and ground truth")


```

QR appears to be more accurate than the others, and LU appears to be the most inaccurate based on the L2 median between the algorithms' outputs and the true vector.
...
mat_size <- 512L
cond_num <- 1e6
metrics <- c("norm", "median", "five_percentile", "ninety_five_percentile", "variance")
...
lu_error <- rel_error_list$lu
qr_error <- rel_error_list$qr
chol_error <- rel_error_list$chol
...
vars <- as.data.frame(do.call(cbind, lapply(list("lu"=lu_error, "qr"=qr_error, "chol"=chol_error), function(x){
  return(x$variance)
})))
vars$metric <- "variance"
vars$iter <- 1:n_sim

metrics <- do.call(rbind, list(norms, meds, five_percentiles, ninety_five_percentiles, vars))
...
Based on the plots, it seems that the errors between the three methods are similar.
...
https://github.com/cindyfang70/hiperglm/pull/4
