...
Both the l2-norm relative difference and the coordinate-wise relative difference show that method via LU decomposition is more accurate than the method via QR decomposition. The method via Cholesky decomposition is the least accurate.

This makes sense because $||x - x_0|| = O(\kappa(A)\epsilon_{machine})$ for solution via LU and QR decomposition, but is $\left[\kappa(A) + \frac{||A(x-x_0)||}{||Ax||}\kappa^2(A)\right]\epsilon_{machine}$ for the method via Cholesky decomposition.
