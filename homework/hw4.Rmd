...
solve_via_cholesky <- function(A, b) {
  L <- chol(A)
  Lx <- forwardsolve(L, b, transpose = TRUE, upper.tri = TRUE)
  x <- backsolve(L, Lx)
  return(x)
}

solve_via_qr <- function(A, b) {
  QR <- qr(A, tol = .Machine$double.eps)
  x <- qr.solve(QR, b)
  return(x)
}
...
cat("The median coordinate-wise relative difference is", median(coor_rel_diff), "\n")
...
rel_diff_cholesky <- norm(x_cholesky - x, type = "2") / norm(x, type = "2")
rel_diff_LU <- norm(x_LU - x, type = "2") / norm(x, type = "2")
rel_diff_qr <- norm(x_qr - x, type = "2") / norm(x, type = "2")

coor_rel_diff_cholesky <- abs(x_cholesky - x) / abs(x)
coor_rel_diff_LU <- abs(x_LU - x) / abs(x)
coor_rel_diff_qr <- abs(x_qr - x) / abs(x)

cat("Relative l2-norm difference (Cholesky):", rel_diff_cholesky, "\n")
cat("Relative l2-norm difference (LU):", rel_diff_LU, "\n")
cat("Relative l2-norm difference (QR):", rel_diff_qr, "\n")

df = data.frame(x_true=x,
                Cholesky=coor_rel_diff_cholesky, 
                LU=coor_rel_diff_LU, 
                QR = coor_rel_diff_qr
)
df = reshape2::melt(df, id.vars="x_true")
ggplot(data= df, aes(x=x_true, y=value, color=variable)) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("Cholesky" = "blue", "LU" = "red", "QR" = "green")) +
  scale_y_log10() +
  labs(y = "Coordinate-wise Relative Difference on log10 scale", 
       x = "True x",
       title = "Coordinate-wise Relative Differences for Cholesky, LU, and QR") +
  theme_minimal()
```

Both the l2-norm relative difference and the coordinate-wise relative difference show that the method from most to least accurate are : **Cholesky, LU, QR**.
...
get_rel_error_list<-function(n_sim=32L, mat_size=512L, cond_num=1e6, 
                             metrics=c("norm", "median", "five_percentile", 
                                       "ninety_five_percentile", "maximum")) {
...
```{r}
visualize_rel_diff<-function(rel_error_list, cond_num){
  metrics = c("norm", "median", "five_percentile", "ninety_five_percentile", "maximum")
  metric_names = c("l2-norm", "Median of Coordinate-wise", "Five Percentile of Coordinate-wise",
                   "Ninety five Percentile of Median of Coordinate-wise", "Maximum of Coordinate-wise")
  i = 1
  metric = metrics[i]
  metric_name = metric_names[i]
  df = data.frame(Cholesky=rel_error_list[["chol"]][[metric]], 
                  LU=rel_error_list[["lu"]][[metric]], 
                  QR =rel_error_list[["qr"]][[metric]]
  )
  df$metric = metric
  for (i in 2:length(metrics)) {
    metric = metrics[i]
    metric_name = metric_names[i]
    new_df = data.frame(Cholesky=rel_error_list[["chol"]][[metric]], 
                        LU=rel_error_list[["lu"]][[metric]], 
                        QR =rel_error_list[["qr"]][[metric]]
    )
  ...
  df = reshape2::melt(df, id.vars="metric")
  
  ggplot(data= df, aes(fill= variable, group=variable, x=log(value))) +
    geom_density(alpha = 0.5) +
    facet_wrap(~metric) +
    labs(title = paste("Condition Number:", cond_num)) +
    theme_minimal()
}
```
...
set.seed(1)
cond_num = 1e6
rel_error_list = get_rel_error_list(cond_num = cond_num)
visualize_rel_diff(rel_error_list, cond_num)
```
...
The general pattern for accuracy is consistent across different condition numbers and different metrics.

  - The accuracy from the highest to the lowest are QR, LU, and Cholesky.  

  - As the condition number increases, the error increases. This is as expected because the error is proportional to the condition number.
  
  - Compared with other metrics, the maximum coordinate-wise relative difference has a larger variance.
...
* Link to the pull request. [https://github.com/Wenxuan-Lu/hiperglm/pull/4]
