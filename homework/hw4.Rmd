...
Cholesky decomposition is fastest (median time: 171 ms), followed by LU decomposition (median time: 212 ms), and then QR decomposition (median time: 521 ms).
...
The relative difference in $\ell^2$-norm is -3.958355e-16.

The coordinate-wise relative differences have mean -2.877685e-16, variance 4.761493e-28, and quantiles outlined in the table above.
...
```{r}
library(tidyverse)
data.frame(Method = c("LU", "LU", "Cholesky", "Cholesky", "QR", "QR"),
                     ...
                     Metric = c("Relative L2", "Euclidean", 
...
```{r}
plot_df <- data.frame(X = x, LU = LU_soln, 
                      Cholesky = Chol_soln, QR = QR_soln) %>%
  as.matrix() %>%
  t()

cmdscale(dist(plot_df), k = 2) %>%
  data.frame() %>%
...
Initially, I used bar charts to visualize the errors, which were summarized using both the Euclidean distance and the Relative L2-norm difference. The two bar charts together offer a clear depiction of the relative sizes of the errors, but they don't provide a spatial context for the results. For that, we need to refer to the second plot.

The second plot is a multi-dimensional scaling of the $x$ vectors obtained from each of the three solving methods. This plot reduces the $x$ points from a 1,024-dimensional space to a 2-dimensional space while maintaining the distances between the points. It's important to note that the y-axis is much more granular than the x-axis, and the labels attached to each point are the actual distances (regardless of spatial orientation). 

Based on these two plots, LU decomposition appears to be the most accurate, being the closest to the "ground truth" $x$ vector. LU decomposition's accuracy is followed by Cholesky decomposition's accuracy, and finally, QR decomposition's accuracy. The solutions from each of these methods seem to be distributed around the ground truth without a clear bias towards a specific direction of error. However, this observation is based on a small sample and may not hold true for larger samples.
