...
**Answer:** The absolute and relative threshold I chose to check convergence is half of the 0.05 quantile of a Chi-squared statistics with df = 1. Near $\hat{\boldsymbol{\beta}}_\textrm{mle}$, an additional Newton's iteration does not move the estimator much. To be specific, it is reasonable to assume that the change in log likelihood due to an additional iteration does not exceed the change due to an additional parameter with no material information. Under the null hypothesis that an additional parameter has no material information, the change in log likelihood follows $\chi^2_1/2$. To be conservative in checking convergence, I chose the 0.05 quantile of it. Overall, the stopping criteria is a smaller-than-$qchisq(0.05, df = 1)/2$ change in the log likelihood between adjacent iterations.
...
**Answer to the bonus question:** I think the idealized convergence criteria does not guarantee the accuracy of the CI estimates. Both a point estimation and a variance-covariance estimation of $\hat{\boldsymbol{\beta}}_\textrm{mle}$ are needed to construct its CI. Though our stopping criteria quantifies the uncertainty in point estimation in the idealized case, it does not quantify how $\epsilon_\textrm{tol}$ influences the variance-covariance estimation. 

## Things to commit/submit

* Link to the pull request. <https://github.com/Wenxuan-Lu/hiperglm/pull/3>
