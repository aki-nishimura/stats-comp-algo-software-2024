...
We note from above that under null $H_0: \beta_j = 0 \text{ for } j = 1, \ldots, p$ :
$$\newcommand{\eqDistribution}{\mathrel{\raisebox{-.2ex}{$\overset{\scalebox{.6}{$\, d$}}{=}$}}}
2 \, \log \frac{
  L \big( \boldsymbol{y} \, | \, \boldsymbol{X}, \boldsymbol{\beta} = \hat{\boldsymbol{\beta}}_\textrm{mle} \big)
}{
...
To get a good estimate for tolerance, we must find the affine surface for loglikelihood in the direction of each element of the canonical basis of $\hat{\beta}$. 

Let us define $\hat{\beta_{-j}}$ as the optimal value of the estimate of $\beta$ in the case where the $j^{th}$ element of the estimate of $\beta$ is set to 0.

$H_0: \beta_j = 0 ; j \in 1, \ldots, p$ :
$$\newcommand{\eqDistribution}{\mathrel{\raisebox{-.2ex}{$\overset{\scalebox{.6}{$\, d$}}{=}$}}}
2 \, \log \frac{
  L \big( \boldsymbol{y} \, | \, \boldsymbol{X}, \boldsymbol{\beta} = \hat{\boldsymbol{\beta}}_\textrm{mle} \big)
}{
  L \big( \boldsymbol{y} \, | \, \boldsymbol{X}, \boldsymbol{\beta} = \beta_{-j} \big)
} \sim \chi^2_1,$$

From the above discussion we know that the difference from the loglikelihood at $\hat{\beta}$ is $Z_j^2 / \, 2$. We repeat the same for all possible values of $j$ in 1 to p. 

Hence to get an estimate for the tolerance along each canonical basis which helps us get an affine p-dimensional surface with edgepoints $E= \{\beta \pm e_j*Z_j^2 / \, 2; j \in 1, \ldots, p\}$. This also gives us a natural estimate for tolerance to be $tol= min_j(Z_j^2 / \, 2)$. We repeat the above experiment a large number of times and choose the value corresponding to quintile 0.05 to get the required value. One can note that this is a more conservative test than just using $\chi^2_1$ at quintile 0.05, and protects from type-1 errors.
...
tol <- quantile(tol_v, probs = 0.05)
tol
```
For sanity check we run the above for p=1 and compare the result with chi-square(df=1) which shows they are essentially the same.
...
In general, While the convergence criteria for optimization algorithms are crucial for ensuring the algorithm's convergence to a solution, they DO NOT directly guarantee the accuracy of resulting confidence interval estimates, even in the asymptotic regime. In other words, the convergence criteria $| f(\boldsymbol{x}\textrm{optim}) - f(\boldsymbol{x}\textrm{curr}) | \leq \epsilon_\textrm{tol}$ do not inherently guarantee the accuracy of resulting confidence interval estimates. Consistency dictates that estimates converge to the true parameter value as sample size increases indefinitely, while asymptotic normality implies estimator distribution approaching normality with increasing sample size. Nevertheless, these properties pertain to the estimator's behavior and may not directly correlate with the objective function's behavior during optimization. Thus, additional conditions, like asymptotic properties of the estimator, are necessary for confidence interval accuracy, necessitating a broader consideration beyond optimization convergence alone. 


## Things to commit/submit

* Link to the pull request- https://github.com/achatto4/hiperglm/pull/3
