...
* Link to the pull request: <https://github.com/acolum/hiperglm/pull/3>
* (Optional) Answer to the bonus question: No, it can't guarantee the accuracy of the resulting confidence interval estimates. Below, I explain why. 

The idealized convergence criteria $| f(\boldsymbol{x}_\textrm{optim}) - f(\boldsymbol{x}_\textrm{curr}) | \leq \epsilon_\textrm{tol}$ is primarily concerned with the accuracy of the point estimate $\hat{\boldsymbol{\beta}}_\textrm{mle}$. However, when it comes to uncertainty quantification and the accuracy of the resulting confidence interval estimates, this criterion alone may not be sufficient, especially in finite samples.

The width of a confidence interval depends not only on the point estimate, but also on the standard error of the estimate, which depends on the second derivative (Hessian) of the log-likelihood at the maximum. In other words, even if the point estimate has converged to within a certain tolerance, the Hessian may not have, and this could affect the standard error and, consequently, the width of the confidence interval.

In the asymptotic regime, under certain regularity conditions, the maximum likelihood estimates are normally distributed, and the Hessian at the maximum converges to the inverse of the Fisher information matrix. Therefore, in this regime, if the convergence criterion is met, it implies that both the point estimate and the Hessian have converged, and the resulting confidence intervals should be accurate.

However, in finite samples, these asymptotic results may not hold, and additional care may be needed to ensure the accuracy of confidence interval estimates. For example, one might consider using bootstrap methods to estimate the standard errors and construct the confidence intervals, as these methods don't rely on asymptotic approximations.

In conclusion, while this convergence criteria can provide some assurance about the accuracy of point estimates and, in the asymptotic regime, confidence intervals, in finite samples, additional methods may be needed to ensure the accuracy of confidence interval estimates.
