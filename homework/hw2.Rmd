...
2 + .Machine$double.eps * 2 == 2 # FALSE, 2 != 2 + 2^-51
```

Similarly, $1 = 2^0$ is recognized as different from $2^0 + 2^0 \times \times 2^{-52}$, but not $1 + 2^{-1} \times 2^{-52}$
...
- on the LHS, $a + b = 1 + e + 1 + e$ which is evaluated to $2 + 2e$, because $2$ is different from $2+2e$, and $(a + b) +c$ is thus evaluated to $3 + 2e$.
...
- $a = 1+e \neq 1$ as proven above, and 
- $b = 0.5 + e \neq 0.5$ since the lowest it can be separated from is $2^{-1} + 2^{-1} \times e$. Therefore, 
- on the LHS, $a+b = 1 + e + 0.5 + e = 1.5  +2e$, $(a+b) + c = 1.5 + 2e + 1 = 2.5 + 2e$. 
- on the RHS, $a + (b+c) = 1 + e + (0.5 + e + 1) = 1 + e + 1.5 + e = 2.5 + 2e$. 
- Therefore, LHS $==$ RHS

```{r}
0.5 + .Machine$double.eps == 0.5 # FALSE, 0.5 + 2^0 * e != 0.5
0.5 + .Machine$double.eps / 2 == 0.5 # FALSE, 0.5 + 2^-1 * e != 0.5
0.5 + .Machine$double.eps / 4 == 0.5 # TRUE, 0.5 + 2^-2 * e == 0.5
```
...
*Explanation*: `sample_var_with_question_mark` is trying to estimate this variance $=1$. However, due to finite precision, this estimate is not accurate at 1, but have large variation. If we change the seed, the estimated variance `sample_var_with_question_mark` can vary a lot, both positive and negative.

The distribution of the estimator `sample_var_with_question_mark` under repeated Monte Carlo simulations is normal distribution with mean 1 by central limit theorem. Below we run the simulation 1000 times and got a normally distributed histogram with large variance.
...
The log error is $log_2(\Delta x^2) = 2 log_2(\Delta x)$, so in theory when plotting the relative error against the log step size, the slope is 2. 

- This is what we observe in the front part (higher step size) of the error plot. The plot shows that there is a linear relationship between log2 step size and log2 relative error with slope at 2. The smaller the log step size, the smaller the log relative error. 

- But the latter part of the plot does not agree with the mathematical theory. The slope in the latter part of the plotted line is increasing, roughly one, and is much less smooth than the earlier part. 
  - The linear increase in error might be due to finite precision of the machine representation. When decreasing the step size, the denominator $2\Delta$ divide by 2. In theory, $f(x + \Delta) - f(x-\Delta)$ in the numerator should also be halved just like in the front part of the plot. However, due to finite precision error, the the numerator $f(x + \Delta) - f(x-\Delta)$ gets so small that it can be not represented accurately by the machine. Everytime the error gets smaller and requires another fraction bit to represent the error, the error gets rounded up or down, losing precision. The log2 of the lost precision divided by step size will give us roughly 1, which explains a linear increase log2 relative error against log step size
