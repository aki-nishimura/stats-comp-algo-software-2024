...
You learned in the class that numeric values in computers are typically represented with 64-bits double-precision numbers, having 53-bits of relative precision. 
In particular, a "Uniform(0, 1)" random variable in double-precision generates a number from the $2^{53}$ unique values between $0$ and $1$.
The amount of randomness in it, therefore, is equivalent to 53 Bernoulli random variables.
Write a function to generate a Uniform(0, 1) random variable in double-precision with randomness coming only from the `rbinom()` function.
...
As we observed above, a Uniform(0, 1) random variable in double-precision takes one of the $2^{53}$ unique values.
Two independently drawn random variables, therefore, has small but positive probability of having exactly the same value.
We can calculate the expected number of duplicate values in i.i.d. Uniform(0, 1) random variables as follows.
...
For the case of double-precision uniform random variables, we have $n = 2^{53}$ for the number of choices.

The `calc_num_expected_duplicate()` function below implements the above formula for the expected number of duplicates. 
When you call `calc_num_expected_duplicate(n_draw = 10^6, n_choice = 2^53)` to calculate the expected number for $10^6$ Uniform(0, 1) random variables, however, the function returns _exact zero_ due to a finite precision error. 
What is the issue?
...
Use the more numerically accurate version of `calc_num_expected_duplicate()` to calculate the expected number of duplicates among $10^6$ Uniform(0, 1) random variables.
...
To test this function, take as an example a quadratic function $f(x) = - \boldsymbol{x}^\intercal \boldsymbol{\Phi} \boldsymbol{x} / 2$ with positive definite $\boldsymbol{\Phi}$, which can be thought of as a log-likelihood function for $x \sim \mathscr{N}(\boldsymbol{0}, \boldsymbol{\Phi})$ up to an additive constant.
First show that its gradient is given by $\nabla f(x) = - \boldsymbol{\Phi} \boldsymbol{x}$; 
more generally, the gradient of a quadratic function $g(x) = \boldsymbol{x}^\intercal \boldsymbol{A} \boldsymbol{x}$ is given by $\nabla g(x) = (\boldsymbol{A} + \boldsymbol{A}^\intercal) \boldsymbol{x}$ for any square matrix $\boldsymbol{A}$.
