...
An important thing about double precision is that there are $2^{52}$ machine-representable values between two reals $2^k$ and $2^{k+1}$.
This means $2$ is not differentiable from $2^1 + 2^{-52}$, but differentiable from $2^1 + 2^1 \times 2^{-52}$.
Prove as follows.

```{r}
2 + .Machine$double.eps == 2
2 + .Machine$double.eps * 2 == 2
```

Similarly, $1 = 2^0$ is differentiable from $2^0 + 2^0 \times \times 2^{-52}$, but not $1 + 2^{-1} \times 2^{-52}$
...
Therefore, in our example, $a + b = 1 + e + 1 + e$ which is evaluated to $2 + 2e$, and $(a + b) +c$ is thus evaluated to $3 + 2e$.
...
```{r}
running_sum_sq / (n_sample-1) - sample_mean * running_sum / (n_sample-1) + n_sample * sample_mean^2 / (n_sample-1)

running_sum_sq / (n_sample) - sample_mean * running_sum / (n_sample) + n_sample * sample_mean^2 / (n_sample)
```

```{r}
(running_sum_sq - n_sample * sample_mean^2) / (n_sample * (n_sample -1))
```


## Things to commit/submit

Answers to the questions above: No the estimate does not make sense. The estimated sample variance using this method is negative. The estimate changes, can be both positive and negative when varying the seeds

The distribution of the estimator `sample_var_with_question_mark` under repeated Monte Carlo simulations is normal distribution with mean 0 by central limit theorem?
...
*Answer*: The relative error plot partially agrees with the mathematical theory. The log relative error is $log_2(\Delta x^2) = 2 log_2(\Delta x)$, and  front part (higher step size) of the error plot agrees that there is a linear relationship between log2 step size and log2 relative error. The smaller the log step size, the smaller the log relative error. 

But the latter part of the plot does not agree with the mathematical theory. The slope in the latter part of the plotted line is increasing, roughly one, and is much less smooth than the earlier part. This might be due to finite precision error of the computer. When doing exponential of two values with small difference, the computer precision may not be able to fully capture the actual difference between the two values, for example $f(x + \Delta) - f(x-\Delta)$ might differ very little very small $\Delta$, but the machine can only represent the small difference till some digit, so rounding off the difference creates greater error with smaller step size. This leads to unstable slope and larger error in the latter part.
