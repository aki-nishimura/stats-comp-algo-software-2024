...
set.seed(615)
x <- rnorm(1) # hmm, the resulting plot looks the same even if x = 1 exactly
...
**Answers:** Since the centered difference approximates the derivative up to $O(\Delta x^2)$, and we are plotting on a log-log scale, I expect the slope to be 2. Indeed, from -17 to 0 (reading from right to left on this plot), the slope is 2.

Meanwhile, the slope from -52 to -17 is much more jagged and appears to be roughly -1 on average. I'm not sure why there is numeric instability here, since all the step sizes are larger than machine epsilon. Maybe we lose precision (i.e., go beyond machine epsilon) when we perform functions like `log()` and `exp()` on these small numbers. Maybe the jaggedness of the line is due to $x$ (which was randomly generated) having fractional parts that get perturbed by our small steps sizes.

Anyhow, if $f(x + \Delta x) - f(x - \Delta x)$ gets truncated to $f(x) - f(x) = 0$, then the centered difference estimate of the derivative at $x$ will be 0, and the relative error will be 1. The log base 2 relative error will be 0. Hmm, this would lead to a slope of 0, not -1. So I am not sure why the slope is -1 from -52 to -17.

<!-- Wrong definition of relative error: relative error = truth/step size \implies log(relative error) = log(truth) - log(step size) \implies change in log(relative error) = log(step size) - log(step size/2) = log(step size)/2. Since change in step size is also log(step size)/2, the slope is 1. -->
