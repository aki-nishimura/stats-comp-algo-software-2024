...
In the first example, the LHS evaluates to $2(1+\epsilon) + 1 = 3 + 2\epsilon$, while the RHS evaluates to $1 + \epsilon + 2 = 3$.
These are not equal because the computer does not distinguish between $\epsilon$ and $0$, but does distinguish between $2\epsilon$ and $0$.

In the second example, the LHS evalutes to $(1 + 0.5 + 2\epsilon) + 1 = 2.5 + 2\epsilon = 2$ and the RHS evaluates to $1 + \epsilon + (1.5 + \epsilon) = 2.5 + 2\epsilon = 2$.
...
The estimate with the original seed does not make sense because the value is -147456 and negative values do not make sense for variance.

The value of the estimator can be either positive or negative depending on the seed.
For example, with a random seed of 140, the estimate is 49152.
...
double_prec_runif <- function () {
  bin <- rbinom(n=52, size=1, prob=0.5)
  fracs <- 1/2^c(1:52)
  decimal <- sum(fracs * bin)
...
The issue is that `1-1/n_choice` evaluates to exactly 1 when `n_choice` is $2^52$ as $2^52$ is the same value as `.Machine$double.eps$`.

```{r}
calc_num_expected_duplicate <- function (n_draw, n_choice) {
  # To be remedied.
  prob_item_never_chosen <-  (n_draw) - (0.5 * (n_draw - 1) *n_draw * (1/n_choice))
  n_expected_unique <- prob_item_never_chosen
...
After fixing the function, the number of expected duplicates is now 0.0001110221.
...
The number of duplicates from the base R uniform sampling is 115, whereas the number of duplicates from our function is 0.
...
I expect the slope of the plotted line to be decreasing as dx decreases.
...
Yes, the relative error plot mostly agrees with the theory as the error is decreasing as dx decreases.
However, after a certain point the error begins to increase despite dx decreasing, likely due to dx being small enough to cause numerical instabilities.

Also, what is the slope in the "latter part" of your plotted line, roughly speaking?
Roughly speaking, the slope at the latter part is 1.
This positive slope could be due to numerical instabilities from dx being small.
...
From Methods Term 1, we know that the gradient of a quadratic form $x^TAx$ where $A$ is symmetric is given by $2Ax$ , so in the case of $-\frac{1}{2}x^T\Phi x$ we get $2\Phi x$.
...
[Pull request](https://github.com/cindyfang70/hiperglm/pull/2)
