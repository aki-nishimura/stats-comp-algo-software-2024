...
calc_num_expected_duplicate <- function (n_draw, n_choice) {
  ...
  d <- n_draw
  n <- n_choice
  prob_item_never_chosen <- 1 - d/n + (1/2) * (1/n^2) * d * (d-1)
  n_expected_unique <- n_choice * (1 - prob_item_never_chosen)
  res <- 2*d - n - (1/(2*n)) * d * (d-1)
  return(res)
}
...
n_choice * (1 - prob_item_never_chosen) is indistinguishable from n_choice (1e+06) due to finite precision arithmetic (1e+06 is between \( 2^{23} \) and \( 2^{24} \), with a distinguishable increment of 199840144), thus the returned result will be 1e+06 - 1e+06 = 0.
The second order taylor expansion of \( (1+x)^d \) is \( (1+x)^d \approx 1+x \times d + \frac{1}{2} \times x^2 \times d \times (d-1)\)
