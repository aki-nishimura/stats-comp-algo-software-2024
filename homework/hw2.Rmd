...
Particularly, when computing running_sum_sq, squaring theta yields extremely large numbers due to the large value of theta combined with random fluctuations from rnorm(1). Summing up these large squared values over 1000 times compounds the precision issues.

Consequently, the calculation of sample_var_with_question_mark is prone to substantial round-off errors, as subtracting a large squared mean from a large second moment is likely to lose significant digits of precision. 
...
The histogram looks quite good! We also check if the data is uniform by running a Kolmogorov Smirnov Test. ALthough the data looks good to the naked eye, the KS test rejects it!
...
1 to $x$ can result in a number that is extremely close to 1(and so are the values obtaied after taking power $n_draw). The multiplication of a very large number with a small number to find $calc_num_expected_duplicate$ also contributeds to the same. In double-precision floating-point representation, where the significand has 52 bits, the representation of numbers very close to 
...
I anticipate that when plotted on a log-log scale, the resulting line will be straight, exhibiting a slope of $-2$. 
