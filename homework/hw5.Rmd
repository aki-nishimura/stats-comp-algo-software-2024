...
One feature of the modern CPU that may be responsible for this is the speculative execution, because this can lead to very efficient run times if the computer guess correctly but slower runtimes when it guesses incorrectly. So depending on the values of the input vector, the guess of the computer may change. This discussion can be found at about 7:54 in the Advanced CPU Designs video above.
...
The code chunk above errors every time I run it because the last inner product disagrees with the previous ones. This is because the C++ code above is trying to access indices outside the range of the arrays, likely resulting in an issue with accessing things in memory that are not relevant to the computation.
...
On the first run, I get a 4x2 matrix with entries in the first column having the value 1024 and the second column being all NaN. On subsequent runs, I get the same result or the second column will contain entries of 512. This is because A and B are of incompatible dimensions, so when we try to index into entries of B that don't exist, we get incorrect and unstable results.
...
The first chunk of code results in the R session being aborted, likely due to memory constraints. This is because the code iterates from 1 to the length of x, but when x is longer than y, this results in memory issues. The second chunk of code runs fine because x is shorter than y.
...
my_matmul <- row_col_dot_matmat(A,B)
R_matmul <- A %*% B
are_all_close(my_matmul, R_matmul)
```
...
Rcpp::sourceCpp(file.path("src", "matmat.cpp"))
my_matmul_col <- col_oriented_matmat(A,B)

R_matmul <- A %*% B
are_all_close(my_matmul_col, R_matmul)
...
When benchmarking `dgemm_eigen()`, I found that having an empty `Makevars` file resulted in a runtime of 138ms on Intel(R) Xeon(R) CPU E5-2650 v4 @ 2.20GHz. When I turned on AVX(2) and FMA, I got a run time of 135ms. Turning off SSE and leaving AVX(2) and FMA on gave a runtime of 134ms.

For `dgemv_eigen()`, I was not able to compile it with SSE turned off. With AVX(2) and FMA turned on, it ran in 5.34ms. With AVX(2) and FMA turned off, it ran in 15.2ms.
