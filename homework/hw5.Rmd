...
**Answer:** I suppose something that could be responsible for this phenomenom is the time it takes to move data from RAM to the CPU (called a "bus" at 5:30 in the "Advanced CPU Designs" video).
...
**Answers:** Sometimes the matrix multiplication yields 1022 (or a number close to it like 1022.0000004141), other times Inf, other times a large number like 3.86e28. I would have hoped the result would be consistently 1023 since that is the size of the matrix. Looking at `inner_prod.cpp`, I think what is going on is that the for loop is using 1-indexing but Cpp uses 0-indexing, so the for loop is skipping the first index of the vector and going past the last index of the vector (at which it may be pulling some crazy value like Inf).
<!-- I have written a 0-indexed function called `correct_inner_prod()` in `matmat.cpp`. -->
...
**Answers::** The first time I ran this chunk, I got a 4x2 matrix where the first column had all entries equal to 1024 and the second column had all entries equal to 3.389e243. On subsequent runs, I got 4x2 matrices where the first column had all entries equal to 1024 while the second column had all entries equal to a really positive number or a really negative number or NaN.

My guess for what is going on is that Rcpp tries to produce a 4x2 matrix (because the outer dimensions of A and B are 4 and 2, respectively) but runs out of data in B in the following way --- my guess is that when computing any entry in the first column of A\*B, the program is taking all 1024 entries in the corresponding row of A and all 1024 entries in B (i.e., both rows of B), which is why the first column of A is 1024. Then, the program runs out of values in B to compute the second column of A\*B, so it uses really positive or really negative numbers (from somewhere in its memory, maybe a junk area?).
...
When I try running the first code chunk, R encounters a fatal error and aborts the session. I believe this happens because `axpy_c.cpp` is trying to access an index of y that is out of bounds, since x has length 10^7 while y has length 10^3.

The second code chunk runs fine because, x has length 10^3 while y has length 10^7, so only the first 10^3 entries of y are modified.

I wrote a new function in `axpy_c.cpp` called `safe_axpy_c()` and tested it in the chunk below.

```{r my-axpy, eval=FALSE}
a <- 3.14
x <- rep(1, 1e7L)
y <- rep(0.00159, 1e3L)

result <- safe_axpy_c(a, x, y)
```
...
result <- row_col_dot_matmat(A, B)
result_baseR <- A %*% B
identical(result, result_baseR)
```
...
**Answer**: Column-oriented implementation should be faster because R stores matrices by columns, so data motion is more efficient that way.

I don't know why my `bench::mark()` isn't working even though my `col_oriented_matmat()` and `row_col_dot_matmat()` functions both work and match up with `%*%`.

```{r col-matmat}
Rcpp::sourceCpp(file.path("src", "matmat.cpp"))

# Check
result <- col_oriented_matmat(A, B)
identical(result, result_baseR)
...
# Import my dgemv_eigen() function
Rcpp::sourceCpp(file.path("src", "dgemv_eigen.cpp"))
```

```{r, eval=TRUE}
# Benchmark RcppEigen implementation of matrix matrix multiplication
bench::mark(dgemm_eigen(A, B))

# Check RcppEigen implementation of matrix vector multiplication
v <- rnorm(n_inner)
result <- dgemv_eigen(A, v)
result_baseR <- A %*% v
all(abs(result - result_baseR) < 1e-10)
all(abs((result - result_baseR) / result_baseR) < 1e-10)

# Benchmark RcppEigen implementation of matrix vector multiplication vs. base R
bench::mark(A %*% v)
