...
This relates to minute 8 in video 3.
In `sign_via_diff()`, the operations in the statement can be performed simultaneously as they are unconditional, however, if has a set of conditions that have to be checked first before performing the operation.
...
The behavior observed is that the calculation isn't deterministic as there was an instance of two runs not giving the same output.
Subseq val returns the value Inf.
The error is due to the way the for loop is initiated in C++.
Indexing starts with 0 in C++, not 1.
Therefore, wrongly initializing with 1, could lead to accessing memory allocated for a value that is not part of the vector and throw error.
Additionally, using `i <= v.size()` will also have us accessing an element out of the vector.
For example, given, length of input vectors = 1023.
Iterating once more through ++i in the for loop has us accessing the 1024th element since we started at index 0, hence throwing an unexpected answer like Inf or -Inf.
Once these errors are corrected to `for (int i = 0; i < v.size(); ++i)`, the code works.
...
I get different results everytime I run it.
...
-   ***Everytime I run the first chunk, RStudio crashes. This could be because, in the first case the length of x was much longer than y. Once our iteration crosses the length of y, similar to exercise 2 part 1, we would be trying to reach memory that is not allocated for vector y hence making the program crash. This is not the case in the second code chunk as we're not accessing elements outside the vector.***
...
```{r}
identical(A%*%B,row_col_dot_matmat(A, B) )
identical(A%*%B,col_oriented_matmat(A, B) )
```
...
\# 1 A %\*% B 351ms 352ms 2.84 4MB 0 2
...
Matrix products: defaultBLAS: /System/Library/Frameworks/Accelerate.framework/Versions/A/Frameworks/vecLib.framework/Versions/A/libBLAS.dylib
...
#   expression       min median `itr/sec` mem_alloc `gc/sec` n_itr
#   <bch:expr>    <bch:> <bch:>     <dbl> <bch:byt>    <dbl> <int>
# 1 dgemm_eigen(… 59.2ms   60ms      16.6       4MB        0     9
