...
required_packages <- c("microbenchmark", "Rcpp")
...
**Solution**
We note that the median time to solve the system using the \texttt{solve} functionality based on LU decomposition is 310 ms, the time to run the cholesky decomposition based methods is 259 ms, and that to run the QR based decomposition is 817 ms. 

The respective complexities are- 
Theoretical complexity:
\begin{itemize}
    \item LU decomposition: $\approx \frac{2}{3}p^3$
    \item Cholesky decomposition $\approx \frac{1}{3}p^3$
    \item QR decomposition: $\approx \frac{4}{3}p^3$
\end{itemize}

The relative speed ratio in our case is roughly 1.2:1:3.5. Ideally(in terms of the complexities) The speed ratio should be 2:1:4. This relative speed between Cholesky and QR decomposition is close to the expected value. The observed difference in speed between Cholesky and LU decomposition diverges from the anticipated ratio; they are closer to 1 than the expected 2. This inconsistency could be due to the need for R to allocate additional memory space for the L matrix after Cholesky decomposition. However it is important to note that the relative ordering of the algorithms w.r.t. the time taken is the same as that expected theoretically.
...
**Solution**
The $\ell^2$-norm is 5.2e-16 ($2^{-50.8}$), while the average coordinate-wise relative differences hover around 1e-15 ($2^{-50}$). These differences align with expectations given the double precision limit the double setup. The relative difference between the two formats is approximately on the order of $10^{-16}$, which aligns with the precision limit of the double format.
...
# Fill in
matv_d=matvec_double(A,x)
matv_ld=matvec_ldouble(A,x)

# Function to calculate the relative difference in l2-norm
relative_l2_norm_difference <- function(v1, v2) {
  norm_diff <- sqrt(sum((v1 - v2)^2))
  norm_v1 <- sqrt(sum(v1^2))
  return(norm_diff / norm_v1)
}

coordinate_wise_relative_difference <- function(v1, v2) {
  return(abs(v1 - v2) / abs(v1))
}

coordinate_wise_abs_difference <- function(v1, v2) {
  return(abs(v1 - v2))
}

relative_l2_norm_diff <- relative_l2_norm_difference(matv_ld, matv_d)

coordinate_wise_rel_diff <- coordinate_wise_relative_difference(matv_ld, matv_d)

cat("Relative difference in l2-norm:", relative_l2_norm_diff, "\n")
cat("Coordinate-wise relative difference:", mean(coordinate_wise_rel_diff), "\n")
cat("Max Coordinate-wise relative difference:", max(coordinate_wise_rel_diff), "\n")
```
...
**Solution**
We find that the Cholesky decomposition based approach is the most accurate, followed by LU and finally QR decomposition. This trend happens in both coordinate wise relative difference as well 
coordinate wise absolute difference. This trend is very important to understand the differentiation within approaches on the basis of the matrix on which it would be used.
```{r}
set.seed(1918)
cond_num <- 1e6
...
est1 <- solve(A, b)
LU_relative_l2_norm_diff <- relative_l2_norm_difference(est1, x)
LU_coordinate_wise_rel_diff <- coordinate_wise_relative_difference(est1, x)
LU_coordinate_wise_abs_diff <- coordinate_wise_abs_difference(est1, x)

est2 <- solve_via_cholesky(A, b)
chol_relative_l2_norm_diff <- relative_l2_norm_difference(est2, x)
chol_coordinate_wise_rel_diff <- coordinate_wise_relative_difference(est2, x)
chol_coordinate_wise_abs_diff <- coordinate_wise_abs_difference(est2, x)

est3 <- qr.solve(A, b)
qr_relative_l2_norm_diff <- relative_l2_norm_difference(est3, x)
qr_coordinate_wise_rel_diff <- coordinate_wise_relative_difference(est3, x)
qr_coordinate_wise_abs_diff <- coordinate_wise_abs_difference(est3, x)

result_df <- data.frame(
  method = rep(c('LU', 'Cholesky', 'QR'), each = length(x)),
  diff = c(LU_relative_l2_norm_diff, chol_relative_l2_norm_diff, qr_relative_l2_norm_diff),
  coordinate_wise_diff = c(mean(LU_coordinate_wise_rel_diff), mean(chol_coordinate_wise_rel_diff), mean(qr_coordinate_wise_rel_diff))
)

all_coordinate_diff <- c(LU_coordinate_wise_rel_diff, chol_coordinate_wise_rel_diff, qr_coordinate_wise_rel_diff)

method_group <- rep(c('LU', 'Cholesky', 'QR'), each = length(x))

coordinate_group <- rep(1:length(x), times = 3)

plot_df <- data.frame(
  method = method_group,
  coordinate_diff = log(all_coordinate_diff),
  coordinate = factor(coordinate_group)
)


boxplot(coordinate_diff ~ method, data = plot_df, ylab = "Coordinate-wise Relative Difference")
...
result_df <- data.frame(
  method = rep(c('LU', 'Cholesky', 'QR'), each = length(x)),
  diff = c(LU_relative_l2_norm_diff, chol_relative_l2_norm_diff, qr_relative_l2_norm_diff),
  coordinate_wise_diff = c(mean(LU_coordinate_wise_abs_diff), mean(chol_coordinate_wise_abs_diff), mean(qr_coordinate_wise_abs_diff))
)

all_coordinate_diff <- c(LU_coordinate_wise_abs_diff, chol_coordinate_wise_abs_diff, qr_coordinate_wise_abs_diff)
...
boxplot(coordinate_diff ~ method, data = plot_df, ylab = "Coordinate-wise Absolute Difference")
...
**Solution**
We note that as the condition number increases so does the error across all three algorithms. Furthermore, we find that the QR algorithm performs the worst while Cholesky usually performs the best(and is comparable with LU based approach). We find that the variance is the highest in the QR based estimation, and is the lowest in the Cholesky based approach, which works best overall. We also note that as expected, the mean and the median show similar results. The 5 and 95 percent results show us relative tail behaviour. One interesting fact is that as the condition number increases, the variance of Cholesky tends to increase towards the LU counterpart. It might point towards more condition number based sensitivity of Cholesky based approach.
...
for (cond_num in cond_numbers) {
  ...
  for (metric in metrics) {
    data <- lapply(c("lu", "chol", "qr"), function(method) rel_error_list[[method]][[metric]])
    boxplot(data, names = c("LU", "Cholesky", "QR"),
            main = paste("Metric:", metric, "| Cond Num:", cond_num),
            ylab = "Relative Error")
  }
}
```
...
* Link to the pull request.- https://github.com/achatto4/hiperglm/pull/4
