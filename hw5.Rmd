...
## Answer
Conditional jump instructions: A jump instruction that depends on the value of the input must wait for the finalized value to perform the jump. This can be sped up using speculative execution by advanced CPUs. This discussion begins at 7:55 of video 3.
...
The bug arises from the indexing used in Rcpp. The loop starts at 1, but Rcpp indexing begins at 0. Therefore, the last element of the result vectoris out of bounds of the loop, so Rcpp does not know which value to put there.
...
I get a 4x2 matrix with 1024 in the first column and NA in the second column. If rerun the code, I sometimes get extremely large values in the second column (but the first column is always 1024). This is happening because Rcpp runs out of values to use in the matrix multiplication, so it is picking random values from RAM to fill in for the second column.
...
## Answer
When I run the first code chunk, my R session aborts. This is similar to the matrix-matrix multiplication situation above, where the y vector is shorter than the ax vector, so Rcpp does not know which values to select from RAM to put in the result.
The second code chunk runs fine because the longer vector is the y vector, so there is no memory issue.
I modified 'axpy_c.cpp' by adding an error if y is longer than x.
...
result_matmat = row_col_dot_matmat(A, B)
result_baseR = A %*% B
```
...
[Link to pull request](https://github.com/sfansler/hiperglm/pull/5)
